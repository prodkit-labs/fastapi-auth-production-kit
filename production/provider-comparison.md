...
| Path | Category | Evidence | Good Fit | Tradeoffs | Watch |
|---|---|---|---|---|---|
| Clerk | Hosted auth UI | `documented` | SaaS apps that want polished hosted UI and account flows | More provider coupling | Pricing at scale, customization depth, migration path |
| Supabase Auth | Hosted auth plus backend platform | `documented` | Apps already using Supabase database, storage, or edge functions | Platform coupling | Email settings, RLS design, local-to-hosted migration |
| Logto | Open-source friendly identity platform | `documented` | Teams that want hosted or self-hosted identity with productized auth flows | Extra service to operate or adopt | Deployment model, upgrade path, integration maturity |
...
| Path | Category | Evidence | Good Fit | Tradeoffs | Watch |
|---|---|---|---|---|---|
| Auth0 | Enterprise identity and federation | `documented` | Enterprise identity, SAML/OIDC, tenant-level administration | Complex configuration | Tenant settings, SAML/OIDC details, cost model |
...
| Path | Category | Evidence | Good Fit | Tradeoffs | Watch |
|---|---|---|---|---|---|
| Resend | Transactional email | `not compared` | Verification and reset emails for developer-focused products | Email-only dependency | Deliverability, domain setup, monitoring |
